...
Deno.serve(async (request) => {
  ...
  try {
    const { question, japanese, mode } = await request.json();
    const input = mode === "conversation" ? japanese : question;
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({ model, input: [{ role: "developer", content: mode === "conversation" ? conversationSystem : phraseSystem }, { role: "user", content: input }], text: { format: { type: "json_object" } } })
    });
