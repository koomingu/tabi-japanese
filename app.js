...
function localAsk(q){let words=q.toLowerCase().split(/\s+/);let p=phrases.map(p=>({p,n:words.reduce((n,w)=>n+((p.cat+p.jp+p.romaji+p.ko).toLowerCase().includes(w)?1:0),0)})).sort((a,b)=>b.n-a.n)[0].p;let keys={물:'식당-0',포장:'식당-6',계산:'식당-7',메뉴:'식당-1',추천:'식당-2',역:'교통-0',택시:'교통-0',막차:'교통-5',가격:'쇼핑-0',얼마:'쇼핑-0',카드:'쇼핑-1',면세:'쇼핑-4',체크인:'숙소-0',체크아웃:'숙소-2',호텔:'숙소-0',와이파이:'숙소-5',화장실:'길 묻기-1',길:'길 묻기-0',사진:'길 묻기-4',여권:'긴급 상황-7',경찰:'긴급 상황-1',병원:'긴급 상황-3',구급차:'긴급 상황-2'};for(let k in keys)if(q.includes(k))p=phrases.find(x=>x.id===keys[k]);let polite=/정중|공손/.test(q)?`<br>더 정중하게: すみません、${p.jp}`:'';$('#ask-answer').innerHTML=`<article class="ai-card"><p class="eyebrow">TABi'S RECOMMENDATION</p><h3>${p.jp}</h3><p class="answer-ko">${p.romaji}<br>${p.ko}</p><small>${p.use}<br>${p.note}${polite}</small><br><button data-play="${p.id}">▶ 일본어로 듣기</button><button class="answer-detail" data-detail="${p.id}">자세히 보기</button></article>`;$('[data-play]').onclick=()=>speak(p.jp);$('[data-detail]').onclick=()=>detail(p.id);event('ai_question',{q,source:'local'})}
